...
(defvar smalltalk-keyword-whitespace " \t\n\f|")
...
(defun smalltalk-end-of-defun ()
  "Move point to the end a method declaration.
After that point is at the right place to start defining a new method."
  (interactive)
  (if (smalltalk-in-bang-syntax)
...
(defun smalltalk-instance-template (with-class)
  "Insert a method.
Name of the method and the category is ask interactively"
  (interactive "p")
...
(defun smalltalk-class-template (with-class)
  "Insert a method on the class side.
Name of the method and the category is ask interactively"
  (interactive "p")
...
(defun smalltalk-private-template (with-class)
  "Insert a private method.
Name of the method is ask interactively
The method is categorized as `private`"
  (interactive "p")
...
(defun smalltalk-forward-keyword-whitespace ()
  "Skip white space and comments forward, stopping at end of buffer
or non-white space, non-comment character"
  (while (looking-at (concat "[" smalltalk-keyword-whitespace "]"))
    (skip-chars-forward smalltalk-keyword-whitespace)
    (if (= (following-char) ?\")
(forward-comment 1))))

(defun smalltalk-backward-keyword-whitespace ()
"Like forward whitespace only going towards the start of the buffer"
(while (progn (skip-chars-backward smalltalk-keyword-whitespace)
(= (preceding-char) ?\"))
(search-backward "\"" nil t 2)))
...
(defun smalltalk-looking-at-unary-send ()
  (looking-at "[A-Za-z][A-Za-z0-9]*[^:A-Za-z0-9]"))
...
(defun smalltalk-previous-keyword-1 ()
  (smalltalk-backward-keyword-whitespace)
...
(defun smalltalk-next-keyword-1 ()
  (smalltalk-forward-keyword-whitespace)
  (if (looking-at "[])};.]")
      nil
